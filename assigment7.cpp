...
int main(int argc, char** argv) {
    std::ofstream output1("Data/output7");
    std::ifstream input("Parameters/input7");
    ...
    for (int t = 0; t < nt - 1; t++) {
        for (int i = 1; i < nx - 1; i++) {
            for (int j = 1; j < ny - 1; j++) {
                u[i][j][t + 1] = u[i][j][t] + nu * dt / (dx * dx) *
                    (u[i + 1][j][t] - 2.0 * u[i][j][t] + u[i - 1][j][t])
                    + nu * dt / (dy * dy) *
                    (u[i][j - 1][t] - 2.0 * u[i][j][t] + u[i][j + 1][t]);
            }
        }
    }
